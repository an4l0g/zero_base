create_extension('vrp/creative3', function()
    ...
    ensure_command('group', function(user_id, group)
        SQL('REPLACE INTO permissions (user_id, permiss) VALUES (?, ?)', { user_id, group })
    end)
    
    ensure_command('ungroup', function(user_id, group)
        SQL('DELETE FROM permissions WHERE user_id=? AND permiss=?', { user_id, group })
    end)
    ...
    ensure_command('addvehicle', function(user_id, vehicle)
        local old = SQL('SELECT * FROM vehicles WHERE user_id=? AND vehicle=?', { user_id, vehicle })
    ...
    ensure_command('delvehicle', function(user_id, vehicle)
        SQL('DELETE FROM vehicles WHERE user_id=? AND vehicle=?', { user_id, vehicle })
    end)
    
    ensure_command('addhouse', function(user_id, name)
        local old = SQL('SELECT * FROM homes WHERE owner=1 AND home=?', { name })[1]
    ...
    ensure_command('delhouse', function(user_id, name)
        SQL('DELETE FROM homes WHERE user_id=? AND home=?', { user_id, name })
    end)
    
    ensure_command('changephone', function(user_id, phone)
        SQL('UPDATE users SET phone=? WHERE id=?', { phone, user_id })
    end)
    
    ensure_command('addgems', function(user_id, gems)
        SQL('UPDATE infos SET gems=gems+? WHERE steam=(SELECT steam FROM users WHERE id=?)', { gems, user_id })
    end)
    
    function vRP.setBanned(user_id, bool)
        SQL('UPDATE infos SET banned=? WHERE steam=(SELECT steam FROM users WHERE id=?)', { bool, user_id })
    end
    ...
    function phone_is_allowed:call(source, form)
        local rows = SQL('SELECT 1 FROM users WHERE phone=?', { form.phone })
        return #rows == 0
    end
end)