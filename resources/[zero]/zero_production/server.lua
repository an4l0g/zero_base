...
sProduction.validateProduction = function(index, amount, cType)
    local _source = source
    local user_id = zero.getUserId(_source)

    local validatedProduction = true
    local materials = configs.products[cType][index].materials
...
sProduction.giveItem = function(index, amount)
    local _source = source
    local user_id = zero.getUserId(_source)
