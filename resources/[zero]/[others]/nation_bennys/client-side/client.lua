        ...
        while (countTable(nearestBlips) > 0) do
            ...
            for index, dist in pairs(_cache) do
                if (dist <= 10 and not nui) then
					...
					DrawMarker(36, config.coord.x, config.coord.y, config.coord.z+0.2, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, 0, 153, 255, 155, 1, 0, 0, 1)
					DrawMarker(27, config.coord.x, config.coord.y, config.coord.z-0.5, 0, 0, 0, 0, 0, 0, 3.0, 3.0, 3.0, 0, 153, 255, 155, 0, 0, 0, 1)
