...
zero.formatWebhook = function(url, title, body)
    ...
    if (url and currentBody and url ~= '') then
		PerformHttpRequest(url, function(err, text, headers) end, 'POST', json.encode({ content = currentBody }), { ['Content-Type'] = 'application/json' })
	end
end