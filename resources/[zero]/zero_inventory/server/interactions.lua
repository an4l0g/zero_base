        ...
        if (user_id) then
            if not (inAction[user_id]) then
                ...
                if (nearestPlayerSource) then
                    local nearestPlayerId = vRP.getUserId(nearestPlayerSource)
                    if not (inAction[nearestPlayerId]) then
                        ...
                        if GetEntityHealth(GetPlayerPed(nearestPlayerSource)) <= 101 then
                            ...
                            if not (vRP.request(nearestPlayerSource, 'Você permite que o mesmo pegue os seus itens?', 60)) then loseSanity(source, user_id) end
                        else
        ...
        if (user_id) then
            if not (inAction[user_id]) then
                ...
                if (nearestPlayerSource) then
                    local nearestPlayerId = vRP.getUserId(nearestPlayerSource)
                    if not (inAction[nearestPlayerId]) then
                        if vRP.request(nearestPlayerSource, 'Você está sendo roubado, deseja aceitar?', 60) then
                            inAction[user_id] = nearestPlayerSource
                            inAction[nearestPlayerId] = true
                            if GetEntityHealth(GetPlayerPed(nearestPlayerSource)) >= 101 then
        ...
        if (user_id) then
            if not (inAction[user_id]) then
                ...
                if (nearestPlayerSource) then
                    local nearestPlayerId = vRP.getUserId(nearestPlayerSource)
                    if not (inAction[nearestPlayerId]) then
                        if vRP.request(nearestPlayerSource, 'Você está sendo revistado, deseja aceitar?', 60) then
                            inAction[user_id] = nearestPlayerSource
                            inAction[nearestPlayerId] = true
                            if GetEntityHealth(GetPlayerPed(nearestPlayerSource)) >= 101 then
                                TriggerClientEvent('zero_inventory:disableActions', nearestPlayerSource)
...
sInventory.callBackInteractions = function(receiveRequest, sendRequest)
    ...
    inAction[vRP.getUserId(receiveRequest)] = nil
    inAction[vRP.getUserId(sendRequest)] = nil
...
sInventory.checkAction = function()
    ...
    if (inAction[vRP.getUserId(source)]) then
        ClearPedTasks(GetPlayerPed(inAction[vRP.getUserId(source)]))
        TriggerClientEvent('zero_inventory:enableActions', inAction[vRP.getUserId(source)])
        inAction[vRP.getUserId(inAction[vRP.getUserId(source)])] = nil
    end
    inAction[vRP.getUserId(source)] = nil
end