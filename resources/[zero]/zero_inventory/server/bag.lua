sInventory.getBag = function(bag_type)
    local items = {}    
    items = vRP.query('zero_inventory:getBag', { bag_type = bag_type })
    if #items > 0 then
...
sInventory.getSlotsByBag = function(bag_type)
    return vRP.query('zero_inventory:getBag', { bag_type = bag_type })[1].slots or {}
end

sInventory.modifyBag = function(bagType, newBag)
    vRP.execute('zero_inventory:updateBagWithoutId', { slots = json.encode(newBag), bag_type = bagType })
end
