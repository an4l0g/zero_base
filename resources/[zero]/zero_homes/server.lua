...
-- zero._prepare('zero_homes/buyHome', 'UPDATE vrp_user_identities SET phone = @phone WHERE user_id = @user_id')
...
srv.tryEnterHome = function(home)
    ...
    if (_userId and home) then
        local consultHome = zero.query('zero_homes/getHomeOwner', { home = home })[1]
        if (consultHome) then
