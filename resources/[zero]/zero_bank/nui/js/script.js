...
function update_money() {
  $.post("https://zero_bank/ws:update_money", JSON.stringify({}), (data) => {
    $("#limit").html(
...
function my_deposit() {
  ...
  if (value) {
    $.post("https://zero_bank/ws:depositar", JSON.stringify({ value: value }));
    closeModal();
    setTimeout(function () {
      update_grafico();
    }, 1000);
...
function withdrawal() {
  let value = parseInt($("#value").val());
  if (value) {
    $.post("https://zero_bank/ws:sacar", JSON.stringify({ value: value }));
    closeModal();
    setTimeout(function () {
      update_grafico();
    }, 1000);
...
function saque_rapido() {
  $.post("https://zero_bank/ws:sacar", JSON.stringify({ value: 1000 }));
  setTimeout(function () {
...
function get_player_trans() {
  $.post("https://zero_bank/ws:get_players_trans", JSON.stringify(), (data) => {
...
function get_trans() {
  $.post("https://zero_bank/ws:get_trans", JSON.stringify({}), (data) => {
...
function clear_trans() {
  $.post("https://zero_bank/ws:limpar_transferencia", JSON.stringify({}));
  $(".list-extract").empty();
  setTimeout(function () {
    update_grafico();
  }, 500);
...
const enviar_transferia = (element) => {
  ...
  let valor = $("#value").val();
  $.post(
    "https://zero_bank/ws:enviar_transferencia",
...
const update_key = () => {
  $.post("https://zero_bank/ws:update_key", JSON.stringify({}), (data) => {
...
const edit_pix_post = () => {
  ...
  if (key.length <= 10 && key.length > 0) {
    $.post("https://zero_bank/edit_pix", JSON.stringify({ key: key }));
    $("modal").hide();
    $("modal").removeClass("modalOpen");
    $(".wrap").css("filter", "blur(0px)");
    setTimeout(function () {
...
const remove_pix = () => {
  $.post("https://zero_bank/remove_pix", JSON.stringify({}));
  setTimeout(function () {
...
const register_pix = () => {
  const key = $("#value").val().trim();
  if (key.length <= 10 && key.length > 0) {
    $.post("https://zero_bank/register_pix", JSON.stringify({ key: key }));
    $("modal").hide();
    $("modal").removeClass("modalOpen");
    $(".wrap").css("filter", "blur(0px)");
    setTimeout(function () {
...
const activeTraffic = (element) => {
  ...
  $.post("https://zero_bank/get_multas", JSON.stringify({}), (data) => {
...
function pay_select_multa(element) {
  let id = element.dataset.id;
  $.post("https://zero_bank/pagar_multa", JSON.stringify({ id_multa: id }));
  $(".pay-traffic").html("");
  setTimeout(function () {
    update_transferencias();
    multas_update();
...
function grafico() {
  let array = [];
  $.post("https://zero_bank/gerate_grafico", JSON.stringify({}), (data) => {
